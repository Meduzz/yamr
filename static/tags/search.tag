<search>
  <div class="container-fluid">
    <div class="alert alert-warning" if={(data==null||data.length==0) && q.value.length > 0}>No data available.</div>
    <div class="alert alert-danger" if={error!=null}>{error}</div>
    <form onsubmit={unsubmit}>
      <div class="form-group">
        <label>Search</label>
        <input type="text" name="q" class="form-control" onkeyup={search}>
      </div>
    </form>
    ...
    <nav>
      <ul class="pager">
        <li class="previous {data.length==0 || page == 0 ? 'disabled' : ''}">
          <a href="#" onclick={prevPage}><span aria-hidden="true">&larr;</span> Previous</a>
        </li>
        <li class="next {data.length==0 ? 'disabled' : ''}">
